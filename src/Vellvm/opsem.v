...
| sReturn : forall S TD Ps F B rid RetTy Result lc gl fs
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B nil (insn_return rid RetTy Result) lc als)::
              (mkEC F' B' (c'::cs') tmn' lc' als')::ECS) EC Mem)
    (mkState ((mkEC F' B' cs' tmn' lc'' als')::ECS) EC Mem')
    E0
...
| sBranch : forall S TD Ps F B lc gl fs bid Cond l1 l2 conds c
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B nil (insn_br bid Cond l1 l2) lc als)::ECS) EC Mem)
    (mkState ((mkEC F (if isGVZero TD c then l2 else l1, 
                       stmts_intro ps' cs' tmn') cs' tmn' lc' als)::ECS) EC Mem)
    E0 
...
| sBranch_uncond : forall S TD Ps F B lc gl fs bid l
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B nil (insn_br_uncond bid l) lc als)::ECS) EC Mem)
    (mkState ((mkEC F (l, stmts_intro ps' cs' tmn') cs' tmn' lc' als)::ECS) EC Mem)
    E0 

| sBop: forall S TD Ps F B lc gl fs id bop sz v1 v2 gvs3 EC ECS cs tmn Mem als,
  BOP TD lc gl bop sz v1 v2 = Some gvs3 ->
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_bop id bop sz v1 v2)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs3) als)::ECS) EC Mem)
    E0 

| sFBop: forall S TD Ps F B lc gl fs id fbop fp v1 v2 gvs3 EC ECS cs tmn Mem als,
  FBOP TD lc gl fbop fp v1 v2 = Some gvs3 ->
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_fbop id fbop fp v1 v2)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs3) als)::ECS) EC Mem)
    E0 
...
| sExtractValue : forall S TD Ps F B lc gl fs id t v gvs gvs' idxs EC ECS cs tmn
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_extractvalue id t v idxs t')::cs) tmn lc als)::ECS)
               EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs') als)::ECS) EC Mem)
    E0
...
| sMalloc : forall S TD Ps F B lc gl fs id t v gns gn align EC ECS cs tmn Mem als
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_malloc id t v align)::cs) tmn lc als) ::ECS) EC Mem)
...
| sFree : forall S TD Ps F B lc gl fs fid t v EC ECS cs tmn Mem als Mem' mptrs mptr,
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_free fid t v)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn lc als)::ECS) EC Mem')
    E0
...
| sAlloca : forall S TD Ps F B lc gl fs id t v gns gn align EC ECS cs tmn Mem als
  ...
  malloc TD Mem tsz gn align = Some (Mem', mb) ->
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_alloca id t v align)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn
                   (updateAddAL _ lc id ($ (blk2GV TD mb) # (typ_pointer t) $))
                   (mb::als))::ECS) EC Mem')
    E0
...
| sLoad : forall S TD Ps F B lc gl fs id t align v EC ECS cs tmn Mem als mps mp gv,
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_load id t v align)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id ($ gv # t $)) als)::ECS) EC Mem)
    E0
...
| sGEP : forall S TD Ps F B lc gl fs id inbounds t v idxs vidxs vidxss EC ECS mp mp'
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_gep id inbounds t v idxs t')::cs) 
                 tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id mp') als)::ECS) EC Mem)
    E0 
...
| sTrunc : forall S TD Ps F B lc gl fs id truncop t1 v1 t2 gvs2 EC ECS cs tmn
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_trunc id truncop t1 v1 t2)::cs) tmn lc als)::ECS)
               EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs2) als)::ECS) EC Mem)
    E0
...
| sExt : forall S TD Ps F B lc gl fs id extop t1 v1 t2 gvs2 EC ECS cs tmn Mem
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_ext id extop t1 v1 t2)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs2) als)::ECS) EC Mem)
    E0
...
| sIcmp : forall S TD Ps F B lc gl fs id cond t v1 v2 gvs3 EC ECS cs tmn Mem als,
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_icmp id cond t v1 v2)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs3) als)::ECS) EC Mem)
    E0
...
| sFcmp : forall S TD Ps F B lc gl fs id fcond fp v1 v2 gvs3 EC ECS cs tmn Mem
  ...
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_fcmp id fcond fp v1 v2)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (updateAddAL _ lc id gvs3) als)::ECS) EC Mem)
    E0
...
| sSelect : forall S TD Ps F B lc gl fs id v0 t v1 v2 cond c EC ECS cs tmn Mem als
  ...
  c @ cond ->
  sInsn (mkCfg S TD Ps gl fs)
    (mkState ((mkEC F B ((insn_select id v0 t v1 v2)::cs) tmn lc als)::ECS) EC Mem)
    (mkState ((mkEC F B cs tmn (if isGVZero TD c
                                then updateAddAL _ lc id gvs2
                                else updateAddAL _ lc id gvs1) als)::ECS) EC Mem)
    E0
...
match state with
| (mkState (nil) ((mkEC _ _ nil (insn_return_void _) _ _)) _ ) => 
...
| (mkState (nil) ((mkEC _ _ nil (insn_return _ _ v) lc _)) _ ) => 
