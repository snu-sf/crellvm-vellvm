  ...
  | LLVMsyntax.Coq_typ_function (t, ts, va) -> 
	...
	| Some n ->
            var_arg_function_type (translate_typ ctx m t) 
              (Array.of_list (List.map (translate_typ ctx m) ts))  
	| None ->
...
let rec translate_constant (ctx:llcontext) (m:llmodule) (c:LLVMsyntax.const) 
  ...
  | LLVMsyntax.Coq_const_arr (t, cs) -> 
      const_array 
        ...
        (Array.of_list (List.map (translate_constant ctx m) cs))
  | LLVMsyntax.Coq_const_struct (_, cs) -> 
      const_struct ctx 
       (Array.of_list (List.map (translate_constant ctx m) cs))
...
let translate_bop bop =
  match bop with
  | LLVMsyntax.Coq_bop_add -> Opcode.Add    
  | LLVMsyntax.Coq_bop_sub -> Opcode.Sub  
  | LLVMsyntax.Coq_bop_mul -> Opcode.Mul
  | LLVMsyntax.Coq_bop_udiv -> Opcode.UDiv
  | LLVMsyntax.Coq_bop_sdiv -> Opcode.SDiv
  | LLVMsyntax.Coq_bop_urem -> Opcode.URem
  | LLVMsyntax.Coq_bop_srem -> Opcode.SRem
  | LLVMsyntax.Coq_bop_shl -> Opcode.Shl
  | LLVMsyntax.Coq_bop_lshr -> Opcode.LShr
  | LLVMsyntax.Coq_bop_ashr -> Opcode.AShr
  | LLVMsyntax.Coq_bop_and -> Opcode.And
  | LLVMsyntax.Coq_bop_or -> Opcode.Or
  | LLVMsyntax.Coq_bop_xor -> Opcode.Xor
                  
let translate_fbop fbop =
  match fbop with
  | LLVMsyntax.Coq_fbop_fadd -> Opcode.FAdd    
  | LLVMsyntax.Coq_fbop_fsub -> Opcode.FSub    
  | LLVMsyntax.Coq_fbop_fmul -> Opcode.FMul    
  | LLVMsyntax.Coq_fbop_fdiv -> Opcode.FDiv    
  | LLVMsyntax.Coq_fbop_frem -> Opcode.FRem    
...
let rec translate_list_value_l vls =
  match vls with
  | (v, l) :: vls' ->  failwith "Not_Supported."
...
let translate_cmd i =
  ...
  | LLVMsyntax.Coq_insn_extractvalue (id, t, v, cs, _) -> failwith "Not_Supported."
  ...
  | LLVMsyntax.Coq_insn_gep (id, inbounds, t, v, vs, t') -> failwith "Not_Supported."
  ...
  | LLVMsyntax.Coq_insn_call (id, noret, tailc, t, _, fid, ps) -> 
      failwith "Not_Supported."
...
let translate_block b =
  match b with
  | (l, LLVMsyntax.Coq_stmts_intro (ps, cs, tmn)) -> failwith "Not_Supported."
