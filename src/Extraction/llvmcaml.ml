...
let coqbop_2_llvmopcode (op:LLVMsyntax.bop) : Llvm.Opcode.t = 
  Coq2llvm.translate_bop op
let coqfbop_2_llvmopcode (op:LLVMsyntax.fbop) : Llvm.Opcode.t = 
...
module GenericValue = struct
  ...
  let rec const2llvalue (td:TargetData.t) gl (c:const) : Llvm.llvalue option = 
    ...
    | Coq_const_struct (_, cs) -> Some (Coq2llvm.translate_constant ctx mm c)
    | Coq_const_gid (_,id) -> 
      begin
    ...
    | Coq_const_fbop (op, c1, c2) ->  
      ...
      | _, _ -> None)
  and list_const2list_llvalue (td:TargetData.t) gl (cs:const list) 
  ...
  let rec llconst2GV (td:TargetData.t) (c:llvalue) : t =   
    ...
    | ValueKind.UndefValue -> ExecutionEngine.get_constant_value c ee 
    | ValueKind.ConstantExpr -> llconstexpr2GV td c
    | ValueKind.ConstantAggregateZero -> ExecutionEngine.get_constant_value c ee
    | ValueKind.ConstantInt -> ExecutionEngine.get_constant_value c ee
    | ValueKind.ConstantFP ->  ExecutionEngine.get_constant_value c ee
    | ValueKind.ConstantArray -> ExecutionEngine.get_constant_value c ee
    | ValueKind.ConstantStruct -> ExecutionEngine.get_constant_value c ee
    | ValueKind.ConstantVector -> failwith "ConstantVector: Not_Supported." 
    | ValueKind.ConstantPointerNull -> ExecutionEngine.get_constant_value c ee
    | ValueKind.GlobalVariable -> ExecutionEngine.get_constant_value c ee
    | ValueKind.Function -> ExecutionEngine.get_constant_value c ee
...
(*    | Opcode.Unwind ->
    ...
    | Opcode.ExtractValue ->      
      failwith "Const Opcode.ExtractValue: Not_Supported"  
    | Opcode.InsertValue ->      
      failwith "Const Opcode.InsertValue: Not_Supported"  
  ...
  *)
  let gep (td:TargetData.t) (t1:LLVMsyntax.typ) (ma:t) (vidxs:t list) 
    (inbounds:bool) (t2:LLVMsyntax.typ) : t option =
    let (ee, mm) = td in
    let ctx = Llvm.module_context mm in
    let ltd = ExecutionEngine.target_data ee in
